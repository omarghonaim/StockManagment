...
const WarehouseItems = ({show,close,wareHouseItems,silp_id}) => {
   ...
   const add_Item_tosilp = ()=>{
    let add = {
      receiving_slip_id : silp_id,
    ...
    return (
                        ...
                        return (
                            <tr key={index}>
                               ...
                               <td>
                                ...
                                <Button className='m-1' onClick={()=>{setShow_Add(true);setadd_item(item)}}>Add</Button>
                                </div>
                               </td>
                            </tr>
                        )
      ...
      <Modal show={show_Add}  onHide={()=>setShow_Add(false)}>
        ...
        <div class="mb-3">
          ...
          <input type="number" class="form-control" id="number" placeholder="Enter QTY" onChange={(e)=>{setadd_item({...add_item,QTY:e.target.value})}} ></input>
         </div>
        </Modal.Body>
        <Modal.Footer>
          <Button onClick={()=>{add_Item_tosilp()}}>add</Button>
        </Modal.Footer>
      </Modal>
        </React.Fragment>
    );
